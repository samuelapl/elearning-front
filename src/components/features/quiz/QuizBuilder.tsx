...
export function QuizBuilder({ course }: QuizBuilderProps) {
  const { updateQuiz } = useLms();
  const [quiz, setQuiz] = useState<Quiz>(() =>
    course.quiz ? JSON.parse(JSON.stringify(course.quiz)) : emptyQuiz(course),
  );
  ...
  const save = () => {
    updateQuiz(course.id, quiz);
    setSaved(true);
  };

  const totalPoints = quiz.questions.reduce((sum, q) => sum + q.points, 0);
  const canSave = quiz.title.trim() !== "" && quiz.questions.length > 0;
