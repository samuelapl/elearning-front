...
import { COURSE_CATEGORIES, userName } from "@/data/mock";
...
export function PendingCourseApprovals() {
  ...
  const pending = useMemo(() => {
    ...
    return courses.filter((course) => {
      ...
      if (!q) return true;
      const owner = userName(users, course.ownerId).toLowerCase();
      return (
  ...
  const confirmReject = () => {
    if (!rejectId) return;
    const result = rejectCourse(rejectId, reason);
    if (!result.ok) {
  ...
  const confirmApprove = (courseId: string) => {
    const result = approveCourse(courseId);
    setFlash(
      result.ok
