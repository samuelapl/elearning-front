...
import { INITIAL_COURSES, INITIAL_SESSIONS, INITIAL_USERS } from "@/data/mock";
...
const FAQS = [
  {
    ...
    answer:
      "No — ELTMS is a front-end prototype that simulates a Learning Management System with realistic mock data. There is no backend; all data lives in the browser and resets when you reload.",
  },
  {
    ...
    <div className="relative mx-auto w-full max-w-3xl">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-white/15 bg-slate-900/80 shadow-2xl shadow-indigo-950/60 backdrop-blur-xl">
        ...
        <div className="grid gap-3 p-5 sm:grid-cols-4">
          {[
            { label: "Courses", value: String(INITIAL_COURSES.length), icon: BookOpen },
            { label: "Staff", value: String(INITIAL_USERS.length), icon: UsersRound },
            { label: "Sessions", value: String(INITIAL_SESSIONS.length), icon: CalendarRange },
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <div className="mx-auto mt-10 flex max-w-lg flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs text-slate-500">
            <span className="inline-flex items-center gap-1.5">
              ...
              {INITIAL_COURSES.length} courses in the catalog
            </span>
            <span className="inline-flex items-center gap-1.5">
              <Check className="h-3.5 w-3.5 text-emerald-400" />
              {INITIAL_USERS.length} staff accounts
            </span>
            <span className="inline-flex items-center gap-1.5">
              <Check className="h-3.5 w-3.5 text-emerald-400" />
